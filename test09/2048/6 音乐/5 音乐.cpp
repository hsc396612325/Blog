...
void win()
{
	...
	drawtext("恭喜你完成了2048，小诚诚决定给你一个摸摸大，作为对你的奖励_(:з」∠)_",&C2,DT_WORDBREAK);
	RECT R1={2,HEIGHT-200,WIDTH-2,HEIGHT-2};
...
{
	mciSendString("close music", NULL, 0, NULL);
	mciSendString("open \"pic\\失败.mp3\" alias music ", NULL, 0, NULL);//背景音乐
	mciSendString("play music", NULL, 0, NULL);
...
void lent()
{
	mciSendString("open \"pic\\背景音乐.mp3\" alias music ", NULL, 0, NULL);//背景音乐
	mciSendString("play music", NULL, 0, NULL);
